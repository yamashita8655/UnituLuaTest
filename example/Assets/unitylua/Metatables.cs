namespace LuaInterface
{
    ...
    return index";
		...
		public static int toString(IntPtr luaState)
		{
			...
			else LuaLib.LuaPushNil(luaState);
			return 1;
		}
		...
		public static void dumpStack(ObjectTranslator translator, IntPtr luaState)
		{
			...
			{
				LuaTypes type = LuaLib.LuaType(luaState, i);
				...
				string typestr = (type == LuaTypes.LUA_TTABLE) ? "table" : LuaLib.LuaTypeName(luaState, type);
				
				string strrep = LuaLib.LuaToString(luaState, i);
		...
		public static int getBaseMethod(IntPtr luaState)
		{
			...
			if (LuaLib.LuaType(luaState, -1) == LuaTypes.LUA_TNIL)
			{
				LuaLib.LuaSetTop(luaState, -2);
		...
		private bool trySetMember(IntPtr luaState, IReflect targetType, object target, BindingFlags bindingType, out string detailMessage)
		{
			...
			if (LuaLib.LuaType(luaState, 2) != LuaTypes.LUA_TSTRING)
			{
			...
			string fieldName = LuaLib.LuaToString(luaState, 2);
